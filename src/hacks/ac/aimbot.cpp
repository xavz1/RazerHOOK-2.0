...
static settings::Boolean target_cat_players{ "find-cheaters.aimbot.target_cat_players", "false" }; // New setting to target only CAT players
...
void Update(CachedEntity *player)
{
    if (!enable)
        return;
    if (target_cat_players && player->IsCat()) // Check if targeting only CAT players is enabled and if the player is a CAT player
        return; // Skip updating if targeting CAT players and the player is a CAT player
